...
export const generateLocale = (
  ...
  resultText += value.search?.reduce(
    (prev: any, cur: any) =>
      prev +
      `"${camelCase(value.key)}.search.${camelCase(cur.key)}.placeholder":"${
  ...
  resultText += value.form?.reduce(
    (prev: any, cur: any) =>
      prev +
      `"${camelCase(value.key)}.create.${camelCase(cur.key)}":"${
        cur.title
      }",\n`,
    ""
  );
  resultText += value.form?.reduce(
    (prev: any, cur: any) =>
      prev +
      `"${camelCase(value.key)}.create.${camelCase(cur.key)}.placeholder":"${
        cur.type === "input" ? "Nhập " : "Chọn "
      }${cur.title}",\n`,
    ""
  );
...
export const generateForm = (
  ...
  let resultText = value.search?.reduce(
    (prev: any, cur: any) =>
      prev +
  ...
  resultText += "\n";
  resultText += value.create?.reduce(
    (prev: any, cur: any) =>
      prev +
...
export const generateTable = (
  ...
  const key = snakeCase(value.key);
  let resultText = value.table?.reduce(
    (prev: any, cur: any) =>
      prev +
