...
export function getDefaultDataOfInterface<T extends JSONObject>(jsonArray: T[]): T {
    const sampleObject = jsonArray.length > 0 ? jsonArray[0] : {};
    const objectInterface: T = {} as T;
    Object.keys(sampleObject).forEach((key) => {
        const value = sampleObject?.[key as keyof typeof sampleObject];
        const valueType = typeof value;
        if (valueType === 'object' && !Array.isArray(value) && value !== null) {
            objectInterface[key as keyof typeof objectInterface] = getDefaultDataOfInterface([value]) as T[keyof T];
        } else {
            objectInterface[key as keyof typeof objectInterface] = value as T[keyof T];
        }
    });
...
export function detectObjectInterface(jsonArray: object[]): object {
    ...
    const interfaceBody = (Array.from(keysAndTypes) as any).reduce((prev: any, [key, type]: any) => ({ ...prev, [key]: type }), {});
    console.log('🚀 >>> interfaceBody:', interfaceBody);
